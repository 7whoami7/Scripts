...
AddEventHandler('esx_orchard:success', function()
	...
	if societyAccount then
		...
		TriggerClientEvent('FeedM:showNotification', xPlayer.source, _U('comp_earned', societyMoney, playerMoney))
	else
		xPlayer.addMoney(total)
		TriggerClientEvent('FeedM:showNotification', xPlayer.source, _U('have_earned', total))
	end
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_orchard', function(inventory)
		...
		if count > 0 and item.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				TriggerClientEvent('FeedM:showNotification', xPlayer.source, _U('player_cannot_hold'))
			else
				inventory.removeItem(itemName, count)
				xPlayer.addInventoryItem(itemName, count)
				TriggerClientEvent('FeedM:showNotification', xPlayer.source, _U('have_withdrawn', count, item.label))
			end
		else
			TriggerClientEvent('FeedM:showNotification', xPlayer.source, _U('quantity_invalid'))
		end
	end)
end)
...
AddEventHandler('esx_orchard:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_orchard', function(inventory)
		...
		if item.count >= 0 then
			...
			TriggerClientEvent('FeedM:showNotification', xPlayer.source, _U('have_deposited', count, item.label))
		else
			TriggerClientEvent('FeedM:showNotification', xPlayer.source, _U('quantity_invalid'))
		end
    ...
    local xPlayer = ESX.GetPlayerFromId(_source)
		...
		local liczba = juice.count
			if juice.count > 0 then
				...
				TriggerClientEvent('FeedM:showNotification', xPlayer.source, "Nie masz soków")
			end
end)
