...
function OpenJuice()
    ...
    if cooldownclick == false then
      if IsPedInAnyVehicle(PlayerPedId()) then
        exports.FeedM:ShowNotification('Wyjdz z pojazdu.')
      else
        cooldownclick = true
        menu.close()
        FreezeEntityPosition(PlayerPedId(), true)
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				if not ESX.Game.IsSpawnPointClear(Config.Zones.VehicleSpawnPoint.Pos, 5.0) then
					exports.FeedM:ShowNotification(_U('spawnpoint_blocked'))
					return
				end
...
function DeleteJobVehicle()
	local playerPed = PlayerPedId()
		...
		if IsInAuthorizedVehicle() then
			...
			exports.FeedM:ShowNotification(_U('only_orchard'))
	end
end
	...
	ESX.TriggerServerCallback('esx_orchard:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					exports.FeedM:ShowNotification(_U('quantity_invalid'))
				else
					menu2.close()
					menu.close()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm',
		{
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_ambulancejob:buyJobVehicle', function (bought)
					if bought then
						exports.FeedM:ShowNotification(_U('vehicleshop_bought', data.current.name, ESX.Math.GroupDigits(data.current.price)))
						...
						exports.FeedM:ShowNotification(_U('vehicleshop_money'))
						menu2.close()
					end
...
function WorkJob3a()
  ...
  if IsPedInAnyVehicle(PlayerPedId(), false) and IsInAuthorizedVehicle() then
    ...
    exports.FeedM:ShowNotification('Musisz byc w aucie')
  end
end
...
AddEventHandler('sadownik:anim2', function()
  exports.FeedM:ShowNotification('Zanieś soki do pojazdu.')
	animacjasoki()
  TriggerEvent('sadownik:petelka')
end)
...
AddEventHandler('sadownik:toomuch', function()
		exports.FeedM:ShowNotification("~r~NIE UDZWIGNIESZ WIECEJ OWOCKÓW")
	ClearPedTasks(PlayerPedId())
end)

RegisterNetEvent('sadownik:toomuchj')
AddEventHandler('sadownik:toomuchj', function()
		exports.FeedM:ShowNotification("~r~NIE UDZWIGNIESZ WIECEJ SOKÓW")
          ...
          local dupcia = GetEntityModel(vehicle)
            for i=1, #Config.AuthorizedVehicles, 1 do
				if dupcia == GetHashKey(Config.AuthorizedVehicles[i].model) then
					...
					exports.FeedM:ShowNotification("Za daleko do pojazdu lub nieautoryzowany model.")
				end
        end
			end
	end
      ...
      DisableControlAction(0, 73, true) -- X
        if IsControlJustReleased(0, Keys['E']) then
          if odlegloscodskupu < 1.5 then
            ...
            exports.FeedM:ShowNotification("Oddaj soki")
          end
        end
    end
end)
...
if not BlipsAdded then
  ...
  table.insert(JobBlips, blipOrchard6)
 	...
 	exports.FeedM:ShowNotification('Masz juz służbowe ciuchy.')
 end
end
